...
1. **頁面匯流與舊 slug**：TSV 是內部台帳、**不能直接貼進 Cloudflare**。
   ...
   補上完整網域、每列 `source_url,target_url,301`；依 Cloudflare 規格**不含 header 列**，檔案恰為 37 個實體列（2026-08-02 加入 `/projects/` 兩列後）。
   **TSV 每次修改後重跑產生器。**
...
| `/projects/`、`/projects/page/1/` → `/research/`（3D 導覽 8→7，Research Portfolio 退役） | 2 | Hugo alias 已生效（掛在 Research hub）；proxy 啟用後轉 301 |
