...
description: "How enquiry and subscription data on sinclairhuang.org is collected, used, retained, and deleted — including the third-party services involved."
...
**Retention, unsubscribing, and deletion.** While your subscription is active, your subscriber data is held in Substack. Every issue carries an unsubscribe link, which stops future newsletter delivery — but unsubscribing does not by itself guarantee that every historical record is deleted straight away. You can ask Sinclair to remove the records he controls from his publication list: write to [research@sinclairhuang.org](mailto:research@sinclairhuang.org) and he will do it from the publisher dashboard. Data that Substack keeps on its own account — for account, security, compliance, or historical purposes — is retained and deleted under Substack's own policy.
...
- **When you submit the enquiry form** — Formspree processes the fields you submit, along with standard request data such as your IP address and browser information. Two anti-spam measures are enabled on this form. **Formshield** is Formspree's own filter: it analyses the submitted content and known spam patterns using machine learning and classifiers. **Google reCAPTCHA**, the built-in option Formspree offers, is operated by Google and performs bot and risk analysis; it may set cookies when it runs, under Google's own policy. A third measure sits in the form itself: a hidden field that automated submissions tend to fill in and people never see.

## Service providers and international transfers

| Service | Role | What it handles |
|---|---|---|
| **Formspree / Formshield** | Processes enquiry form submissions on Sinclair's behalf | The fields you submit and standard request data; Formshield analyses the message content and known spam patterns |
| **Google reCAPTCHA** | Bot and risk analysis on the enquiry form, enabled through Formspree | Performs the analysis when it runs and may set cookies at that point, under Google's own policy |
...
This notice is versioned by date. Enquiries record the version in force when they were sent, so it is always clear which terms applied. The current version is shown at the top of this page.
